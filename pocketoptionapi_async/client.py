...
class AsyncPocketOptionClient:
    ...
    def __init__(
        ...
        self._latest_payouts: Dict[str, float] = {}
        self._latest_order_info: Optional[str] = None
        ...
        self._websocket._parent_client = self  # ربط غير مباشر من الخارج
        self._websocket._parent_client2 = self  # ربط غير مباشر من الخارج
        self._websocket._parent_client3 = self  # ربط غير مباشر من الخارج
    ...
    async def pyout(self, symbol: str) -> Optional[float]:
        """
        Return latest payout for the given asset symbol.
        """
        payout = self._latest_payouts.get(symbol)
        return payout
    
    
    async def Checkwin2(self, deal_id: str) -> Optional[str]:
        """
        Returns the result and profit of a deal by ID.
        """
        deal = self._websocket._deals_cache.get(str(deal_id))
        if deal:
    ...
    async def place_order(
        ...
        try:
            # Reset latest order info before placing a new order
            self._latest_order_info = None
            ...
            order = Order(
                ...
                direction=direction,
                duration=duration,
                request_id=None  # Leave as None if not required
            )

            # Send the order to the server
            await self._send_order(order)

            # Wait until latest_order_info is updated (i.e., server responded with order ID)
            timeout = 5  # seconds
            start_time = time.time()
            while self._latest_order_info is None and (time.time() - start_time) < timeout:
                await asyncio.sleep(0.1)

            if self._latest_order_info is None:
                raise Exception("❌ Order ID was not received from the server within the timeout period.")

            order_id = self._latest_order_info
            #print(f"🧾 Current value of _latest_order_info: {order_id}")

            # Just return the order object for test printing
    ...
    async def _request_candles2(self, asset: str, timeframe: int):
        """Request candle data and print responses without stopping on timeout"""
        ...
        try:
            ...
            # ✅ ما نوقفش البرنامج: نرجع [] ونسيب الـ client يكمل استلام ticks عادي
            return []
    ...
    def _parse_candles_data(self, data: List[Any], asset: str, timeframe: int):
        """Parse tick data into candles with proper time alignment"""
        candles = {}
        ...
        try:
            ...
            # Process all ticks
            for tick in data:
                if len(tick) < 2:  # Skip invalid ticks
                    continue
                    
                timestamp = datetime.fromtimestamp(tick[0])
                # Align to candle timeframe (e.g. minute start for 1m candles)
            ...
            result = []
            for candle_time, values in candles.items():
    ...
    async def _on_json_data(self, data: Dict[str, Any]) -> None:
        ...
        ticks = data.get('history', [])  # Assuming ticks come in 'history' field

        if not asset or not period or not ticks:
            return

        request_id = f"{asset}_{period}"
        
        if (hasattr(self, '_candle_requests') and 
            request_id in self._candle_requests and 
            not self._candle_requests[request_id].done()):
            
            candles = self._parse_candles_data(ticks, asset, period)
            
            if candles:
                self._candle_requests[request_id].set_result(candles)
                if self.enable_logging:
                    logger.success(f"Received {len(candles)} candles for {asset}")
                    del self._candle_requests[request_id]
                    return
            return
